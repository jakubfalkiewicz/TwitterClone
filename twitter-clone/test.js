  ...
  for (const yearMonth in expenses) {
    ...
    for (const [_, expense] of dates) {
      ...
      for (const category in expense) {
        dayExpenses += expense[category].reduce((acc, curr) => acc + curr, 0);
      }
      monthExpenses.push(dayExpenses);
    }
  }
  for (
    let i = 0;
    i < Object.entries(expenses).length - monthExpenses.length;
    i++
  ) {
    monthExpenses.push(0);
  }
